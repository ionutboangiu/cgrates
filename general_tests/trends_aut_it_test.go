...
func testtrendAuQLoadConfig(t *testing.T) {
	...
	if trendAuQCfg, err = config.NewCGRConfigFromPath(context.Background(), trendAuQCfgPath); err != nil {
		t.Error(err)
	}
}

func testtrendAuQInitDataDb(t *testing.T) {
	if err := engine.InitDB(trendAuQCfg); err != nil {
		t.Fatal(err)
	}
}
...
func testtrendAuQRpcConn(t *testing.T) {
	trendAuQRpc = engine.NewRPCClient(t, trendAuQCfg.ListenCfg(), *utils.Encoding)
}
