...
func setTestCache(dm *DataManager, cfg *config.CGRConfig, locker *guardian.GuardianLocker) *CacheS {
	cacheS := NewCacheS(cfg, dm, dm.connMgr, nil, locker)
	dm.SetCache(cacheS)
	if dm.connMgr != nil {
		dm.connMgr.SetCache(cacheS)
	}
	return cacheS
}
...
func TestUpdateFilterIHMisingIndxHasNotReplyIndexes(t *testing.T) {
	...
	if err := cacheS.Set(context.Background(), utils.CacheRateFilterIndexes, "cgrates.org:RP:*none:*any:*any", ssRt, []string{}, true, utils.NonTransactional); err != nil {
		t.Error(err)
	}
...
func TestUpdateFilterIHMisingIndxGetFilterAsIndexSetErr(t *testing.T) {
	...
	if err := cacheS.Set(context.Background(), utils.CacheFilters, "cgrates.org:fltr", flt, []string{}, true, utils.NonTransactional); err != nil {
		t.Error(err)
	}
...
func TestGetFltrIdxHealthGetKeysForPrefixErr(t *testing.T) {
	...
	if _, err := GetFltrIdxHealth(context.Background(), dm, useLtcache, useLtcache, useLtcache, utils.CacheVersions); err == nil || err.Error() != expErr {
		t.Errorf("Expected error <%v>, Received error <%v>", expErr, err)
	}
...
func TestGetFltrIdxHealthgetIHObjFromCacheErr(t *testing.T) {
	...
	if err := cacheS.Set(context.Background(), utils.CacheAttributeProfiles, "cgrates.org:ATTR1", nil, []string{}, true, utils.NonTransactional); err != nil {
		t.Error(err)
	}
...
func TestGetRevFltrIdxHealthFromObjGetKeysForPrefixErr(t *testing.T) {
	...
	expErr := "unsupported prefix in GetKeysForPrefix: "
	if _, err := getRevFltrIdxHealthFromObj(context.Background(), dm, useLtcache, useLtcache, useLtcache, utils.CacheVersions); err == nil || err.Error() != expErr {
		t.Errorf("Expected error <%v>, Received error <%v>", expErr, err)
	}
