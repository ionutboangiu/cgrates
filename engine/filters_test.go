...
func TestRatesCostFiltering(t *testing.T) {
	...
	dmFilterPass.SetCache(NewCacheS(cfg, nil, nil, nil, locker))
	filterS := FilterS{
		cfg: cfg,
		dm:  dmFilterPass,
	}
...
func TestFilterTrends(t *testing.T) {
	...
	dp := NewDynamicDP(context.Background(), cfg, "cgrates.org", initDP, fS)
...
func TestFilterRanking(t *testing.T) {
	...
	dp := NewDynamicDP(context.Background(), rankCfg, "cgrates.org", initDP, fS)
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
