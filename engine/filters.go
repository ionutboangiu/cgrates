...
func (fltr *FilterRule) CompileValues() (err error) {
	...
	case utils.MetaActivationInterval, utils.MetaNotActivationInterval:
		...
		for i, strVal := range fltr.Values {
			if fltr.rsrValues[i], err = utils.NewRSRParser(strVal, config.CgrConfig().GeneralCfg().RSRSep); err != nil {
				return
			}
		}
	case utils.MetaNever: //return since there is not need for the values to be compiled in this case
		return
	default:
		if fltr.rsrValues, err = utils.NewRSRParsersFromSlice(fltr.Values, config.CgrConfig().GeneralCfg().RSRSep); err != nil {
			return
		}
	}
	if fltr.rsrElement, err = utils.NewRSRParser(fltr.Element, config.CgrConfig().GeneralCfg().RSRSep); err != nil {
		return
