...
func TestACExecuteAccountsSetBalance(t *testing.T) {
	...
	actCdrLG.config.ActionSCfg().Conns[utils.MetaAccounts] = []*config.DynamicConns{{ConnIDs: []string{utils.ConcatenatedKey(utils.MetaInternal, utils.MetaAccounts)}}}
...
func TestACExecuteAccountsRemBalance(t *testing.T) {
	...
	actRemBal.config.ActionSCfg().Conns[utils.MetaAccounts] = []*config.DynamicConns{{ConnIDs: []string{utils.ConcatenatedKey(utils.MetaInternal, utils.MetaAccounts)}}}
	ctx, cancel := context.WithTimeout(context.Background(), 10)
	expected = context.DeadlineExceeded.Error()
