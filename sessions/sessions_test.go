...
var attrs = &attributes.ProcessEventReply{
...
func TestV1ProcessEventReplyAsNavigableMap(t *testing.T) {
	...
	//attributes check
	v1per.Attributes = make(map[string]*attributes.ProcessEventReply)
...
`)
	...
	if err := sS.BiRPCv1STIRIdentity(nil, &V1STIRIdentityArgs{
		...
		t.Error(err)
	} else if err := AuthStirShaken(context.Background(), cacheS, rcv, "1001", "", "1002", "", utils.NewStringSet([]string{utils.MetaAny}), -1, sS.cfg.GeneralCfg().ReplyTimeout); err != nil {
		t.Fatal(err)
	}
}
