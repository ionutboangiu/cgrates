...
cgrates.org,DEFAULT,,,*default,*none,0`,
	...
	time.Sleep(time.Second) // wait for DiameterAgent service to start
	diamClient, err := NewDiameterClient(cfg.DiameterAgentCfg().Listen, "localhost",
		cfg.DiameterAgentCfg().OriginRealm, cfg.DiameterAgentCfg().VendorID,
		cfg.DiameterAgentCfg().ProductName, utils.DiameterFirmwareRevision,
		cfg.DiameterAgentCfg().DictionariesPath, cfg.DiameterAgentCfg().ListenNet)
...
func TestDiameterReplication(t *testing.T) {
	// b.Skip("still incomplete")
	var dbCfg engine.DBCfg
	switch *utils.DBType {
	case utils.MetaInternal:
...
cgrates.org,DEFAULT,,,*default,*none,0`,
	...
	var sent int64
	var answered, completed atomic.Int64
	sendCCR := func(t testing.TB, wg *sync.WaitGroup, replyTimeout time.Duration, wantResultCode string) {
		defer wg.Done()
		mu.Lock()
		sent++
		sessionID := fmt.Sprintf("session%d", sent)
	...
	for range reqNo {
		wg.Add(1)
		go sendCCR(t, &wg, 20*time.Second, "2001")
	}
	wg.Wait()
	time.Sleep(10 * time.Second)

	// check results
	t.Logf("sent %d, answered %d, completed %d", sent, answered.Load(), completed.Load())
	var acnt *engine.Account
	attrsAcnt := &utils.AttrGetAccount{
		Tenant:  "cgrates.org",
		Account: "1001",
	...
	if err = client.Call(context.Background(), utils.APIerSv2GetAccount, attrsAcnt, &acnt); err != nil {
		...
		t.Errorf("APIerSv1.GetAccount: sms_balance: %f, want: %f", rply, expBalance)
	}
	if err = rplClient.Call(context.Background(), utils.APIerSv2GetAccount, attrsAcnt, &acnt); err != nil {
		t.Errorf("APIerSv1.GetAccount unexpected err: %v", err)
	} else if rply := acnt.BalanceMap[utils.MetaSMS].GetTotalValue(); rply != expBalance {
