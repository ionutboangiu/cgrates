...
func TestParseEventValue(t *testing.T) {
	...
	config.SetCgrConfig(cfg)
	ev := NewFSEvent(hangupEv)
	if tor, _ := ev.ParseEventValue(utils.ToR, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.ToR, utils.InfieldSep), ""); tor != utils.MetaVoice {
		t.Errorf("Unexpected tor parsed %q", tor)
	}
	if accid, _ := ev.ParseEventValue(utils.OriginID, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.OriginID, utils.InfieldSep), ""); accid != "e3133bf7-dcde-4daf-9663-9a79ffcef5ad" {
		t.Error("Unexpected result parsed", accid)
	}
	if parsed, _ := ev.ParseEventValue(utils.OriginHost, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.OriginHost, utils.InfieldSep), ""); parsed != "10.0.3.15" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Source, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Source, utils.InfieldSep), ""); parsed != "FS_EVENT" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.RequestType, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.RequestType, utils.InfieldSep), ""); parsed != utils.MetaPrepaid {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Tenant, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Tenant, utils.InfieldSep), ""); parsed != "cgrates.org" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Category, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Category, utils.InfieldSep), ""); parsed != "call" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.AccountField, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.AccountField, utils.InfieldSep), ""); parsed != "1001" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Subject, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Subject, utils.InfieldSep), ""); parsed != "1001" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Destination, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Destination, utils.InfieldSep), ""); parsed != "1003" {
		t.Error("Unexpected result parsed", parsed)
	}
	...
	if parsed, _ := ev.ParseEventValue(utils.SetupTime, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.SetupTime, utils.InfieldSep), ""); parsed != sTime.String() {
	...
	if parsed, _ := ev.ParseEventValue(utils.AnswerTime, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.AnswerTime, utils.InfieldSep), ""); parsed != aTime.String() {
		t.Errorf("Expecting: %s, parsed: %s", aTime.String(), parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Usage, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Usage, utils.InfieldSep), ""); parsed != "66000000000" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.PDD, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.PDD, utils.InfieldSep), ""); parsed != "0.028" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Route, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Route, utils.InfieldSep), ""); parsed != "supplier1" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.RunID, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.RunID, utils.InfieldSep), ""); parsed != utils.MetaDefault {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue(utils.Cost, utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+utils.Cost, utils.InfieldSep), ""); parsed != "-1" {
		t.Error("Unexpected result parsed", parsed)
	}
	if parsed, _ := ev.ParseEventValue("Hangup-Cause", utils.NewRSRParserMustCompile(utils.MetaDynReq+utils.NestingSep+"Hangup-Cause", utils.InfieldSep), ""); parsed != "NORMAL_CLEARING" {
		t.Error("Unexpected result parsed", parsed)
	}
}
